...
public class TaiKhoanController : ControllerBase
{
    ...
    {
        Console.WriteLine($"[DEBUG Controller] CapNhat - id={id}, dto={System.Text.Json.JsonSerializer.Serialize(dto)}");
        var userId = GetUserId();
        if (userId == null)
            return Unauthorized(ApiResponse.Fail("Token không hợp lệ"));

        var thanhCong = await _taiKhoanBll.CapNhatAsync(id, dto, userId.Value, ct);
        Console.WriteLine($"[DEBUG Controller] CapNhat result: {thanhCong}");
        if (!thanhCong)
    ...
    [ProducesResponseType(typeof(ApiResponse), StatusCodes.Status200OK)]
    [ProducesResponseType(typeof(ApiResponse), StatusCodes.Status404NotFound)]
    public async Task<ActionResult<ApiResponse>> CapNhatTrangThai(int id, [FromBody] CapNhatTrangThaiTaiKhoanDto dto, CancellationToken ct = default)
    {
        var userId = GetUserId();
        if (userId == null)
            return Unauthorized(ApiResponse.Fail("Token không hợp lệ"));

        var thanhCong = await _taiKhoanBll.CapNhatTrangThaiAsync(id, userId.Value, dto.TrangThai, ct);
        if (!thanhCong)
            return NotFound(ApiResponse.Fail("Không tìm thấy tài khoản"));
...
public class CapNhatTrangThaiTaiKhoanDto
{
    public int TrangThai { get; set; } // 1 = Đang dùng, 0 = Ẩn
}
